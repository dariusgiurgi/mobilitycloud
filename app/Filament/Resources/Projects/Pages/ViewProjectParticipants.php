...
class ViewProjectParticipants extends Page
{
    ...
    public string $filterMobilityId = '';
    ...
    public function getParticipants()
    {
        ...
        return $all->filter(function (Participant $p) {
            ...
            if ($this->filterMobilityId !== '' && ! $p->mobilities->contains('id', (int) $this->filterMobilityId)) {
                return false;
            }
    ...
    {
        return collect([
            $this->filterSearch, $this->filterRole, $this->filterCountry, $this->filterOrg, $this->filterMobilityId,
        ])->filter(fn ($v) => $v !== '')->count()
    ...
    public function getMobilityRegistrationUrl(int $mobilityId): ?string
    {
        $mobility = $this->record->mobilities()->find($mobilityId);

        if (! $mobility?->hasActiveParticipantRegistrationLink()) {
            return null;
        }

        return route('public.participant-registration.show', $mobility->participant_registration_token);
    }

    public function createMobilityRegistrationLink(int $mobilityId): void
    {
        $mobility = $this->record->mobilities()->find($mobilityId);
        abort_unless($mobility, 404);
        $this->authorizeManagementModuleMutation('participants', 'mobility-registration-link:'.$mobility->id, 'Mobility registration link');
        ...
        Notification::make()
            ->title('Mobility registration link created')
            ->body('This link registers people only for '.$mobility->name.'.')
            ->success()
            ->send();
    }

    public function closeMobilityRegistrationLink(int $mobilityId): void
    {
        $mobility = $this->record->mobilities()->find($mobilityId);
        abort_unless($mobility, 404);
        $this->authorizeManagementModuleMutation('participants', 'mobility-registration-link:'.$mobility->id, 'Mobility registration link');

        $mobility->forceFill(['participant_registration_closed_at' => now()])->save();

        Notification::make()
            ->title('Mobility registration link closed')
            ->success()
            ->send();
    }
