@php
    /** @var \App\Models\Workspace $record */
...
<div class="space-y-5">
    <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Access state</p>
                <div class="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-sm text-gray-600 dark:text-gray-300">
                        {{ $planOptions[$record->plan] ?? ucfirst((string) $record->plan) }} · {{ $record->subscriptionStatusLabel() }}
                    </span>
                </div>
            </div>
            <div class="text-right text-sm text-gray-600 dark:text-gray-300">
                <div>{{ $record->users_count ?? $record->users()->count() }} user(s)</div>
                <div>{{ $record->projects_count ?? $record->projects()->count() }} project(s)</div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 p-4 dark:border-white/10">
        <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <span class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700 dark:bg-white/10 dark:text-gray-200">
                {{ $record->billing_provider ? (\App\Filament\Resources\PlatformWorkspaces\PlatformWorkspaceResource::billingProviderOptions()[$record->billing_provider] ?? str($record->billing_provider)->replace('_', ' ')->title()) : 'Not connected' }}
            </span>
        </div>
        ...
        <div class="mt-3 grid gap-3 md:grid-cols-3">
            <div>
                ...
                <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $record->billing_interval ? (\App\Filament\Resources\PlatformWorkspaces\PlatformWorkspaceResource::billingIntervalOptions()[$record->billing_interval] ?? str($record->billing_interval)->title()) : '—' }}</p>
            </div>
            <div>
