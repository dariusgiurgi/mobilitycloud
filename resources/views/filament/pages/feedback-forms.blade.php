<x-filament-panels::page>
    ...
    @php
        ...
        $viewingCampaign = $this->viewingCampaign;
        $campaignResults = $this->campaignResults;
    @endphp
    <style>
        .mc-feedback-grid{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(310px,.95fr);gap:1rem;align-items:start}.mc-feedback-list{display:grid;gap:.55rem}.mc-feedback-card{border:1px solid rgba(148,163,184,.22);border-radius:.85rem;padding:.8rem;background:rgba(255,255,255,.65)}.mc-feedback-actions{display:flex;gap:.38rem;flex-wrap:wrap;align-items:center}.mc-feedback-chip{display:inline-flex;align-items:center;padding:.18rem .42rem;border-radius:99px;background:#eef2ff;color:#4338ca;font-size:.61rem;font-weight:750}.mc-feedback-sub{color:#64748b;font-size:.68rem;line-height:1.45}.mc-feedback-question{padding:.7rem;border:1px solid rgba(148,163,184,.2);border-radius:.7rem;background:rgba(248,250,252,.74);margin-top:.5rem}.mc-feedback-fields{display:grid;grid-template-columns:1fr 1fr;gap:.55rem}.mc-feedback-field label{display:block;color:#64748b;font-size:.6rem;font-weight:800;letter-spacing:.04em;text-transform:uppercase;margin-bottom:.22rem}.mc-feedback-field input,.mc-feedback-field textarea,.mc-feedback-field select{width:100%;border:1px solid rgba(100,116,139,.28);border-radius:.5rem;padding:.47rem .55rem;background:transparent;font-size:.73rem}.mc-feedback-field textarea{min-height:70px;resize:vertical}.mc-feedback-modal{position:fixed;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;padding:1rem;background:rgba(15,23,42,.45)}.mc-feedback-modal-panel{width:min(860px,100%);max-height:calc(100vh - 2rem);overflow:auto;border-radius:1rem;background:white;box-shadow:0 25px 70px rgba(15,23,42,.28);padding:1rem}.mc-feedback-result{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:.5rem}.mc-feedback-result-item{padding:.6rem;border:1px solid rgba(148,163,184,.18);border-radius:.65rem;background:#f8fafc}.mc-feedback-empty{padding:1.4rem;text-align:center;color:#64748b;font-size:.75rem;border:1px dashed rgba(100,116,139,.3);border-radius:.8rem}@media(max-width:900px){.mc-feedback-grid{grid-template-columns:1fr}}@media(max-width:600px){.mc-feedback-fields{grid-template-columns:1fr}}
    </style>
    ...
    @if($showResultsModal && $viewingCampaign)
        ...
        <div class="mc-feedback-modal" role="dialog" aria-modal="true"><div class="mc-feedback-modal-panel"><div style="display:flex;justify-content:space-between;gap:.8rem;align-items:flex-start;margin-bottom:.85rem;"><div><h2 style="font-size:1rem;font-weight:800;margin:0;">{{ $viewingCampaign->title }}</h2><p class="mc-feedback-sub" style="margin:.2rem 0 0;">{{ $viewingCampaign->mobility?->project?->name }} · {{ $viewingCampaign->mobility?->name }}</p></div><button wire:click="closeModal('results')" type="button" style="border:0;background:transparent;font-size:1.2rem;cursor:pointer;">×</button></div><div style="display:flex;gap:.4rem;align-items:center;flex-wrap:wrap;padding:.62rem;border:1px solid rgba(99,102,241,.18);border-radius:.65rem;background:#f8faff;"><input readonly value="{{ $feedbackLink }}" style="flex:1;min-width:220px;border:0;background:transparent;font-size:.7rem;outline:0;"><button type="button" x-data="{ copied:false }" x-on:click="navigator.clipboard && window.isSecureContext ? navigator.clipboard.writeText('{{ $feedbackLink }}') : null; copied=true; setTimeout(() => copied=false, 1500)" style="padding:.36rem .5rem;border:1px solid rgba(100,116,139,.25);border-radius:.45rem;background:white;cursor:pointer;font-size:.66rem;font-weight:700;"><span x-text="copied ? 'Copied' : 'Copy link'">Copy link</span></button><a href="{{ $feedbackLink }}" target="_blank" style="padding:.36rem .5rem;border:1px solid rgba(100,116,139,.25);border-radius:.45rem;background:white;color:inherit;text-decoration:none;font-size:.66rem;font-weight:700;">Open</a></div><div style="display:flex;justify-content:space-between;gap:.6rem;align-items:center;flex-wrap:wrap;margin:1rem 0 .55rem;"><div><strong style="font-size:.82rem;">{{ $viewingCampaign->responses->count() }} anonymous {{ \Illuminate\Support\Str::plural('response', $viewingCampaign->responses->count()) }}</strong><div class="mc-feedback-sub">Results are grouped by question. No response is connected to a participant.</div></div><a href="{{ route('feedback-campaigns.export', $viewingCampaign) }}" style="padding:.4rem .55rem;border:1px solid rgba(100,116,139,.25);border-radius:.45rem;color:inherit;text-decoration:none;font-size:.68rem;font-weight:700;">Export CSV</a></div>@if($campaignResults['ratings']->isNotEmpty())<div class="mc-feedback-result">@foreach($campaignResults['ratings'] as $rating)<div class="mc-feedback-result-item"><div class="mc-feedback-sub">{{ $rating['label'] }}</div><div style="font-size:1.1rem;font-weight:820;margin-top:.2rem;">{{ $rating['average'] ?? '—' }}<span style="font-size:.65rem;color:#64748b;"> / 5</span></div><div class="mc-feedback-sub">{{ $rating['count'] }} ratings</div></div>@endforeach</div>@endif @if($campaignResults['comments']->isNotEmpty())<div style="margin-top:1rem;"><div style="font-size:.8rem;font-weight:780;margin-bottom:.4rem;">Anonymous comments</div>@foreach($campaignResults['comments'] as $comment)<div style="padding:.58rem .65rem;border-top:1px solid rgba(148,163,184,.18);"><div class="mc-feedback-sub" style="font-weight:750;">{{ $comment['label'] }}</div><div style="font-size:.72rem;line-height:1.45;margin-top:.15rem;white-space:pre-wrap;">{{ $comment['answer'] }}</div></div>@endforeach</div>@endif</div></div>
    @endif
</x-filament-panels::page>
