...
class ContentBlockSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (! $workspace) {
            $this->command?->warn('ContentBlockSeeder: no workspace found, skipped.');
            return;
        }
...
TXT],
        ...
        foreach ($blocks as $block) {
            ContentBlock::updateOrCreate(
                ['workspace_id' => $workspace->id, 'title' => $block['title']],
                [
        ...
        $this->command?->info('Seeded ' . count($blocks) . ' proven content blocks into workspace #' . $workspace->id . '.');
    }
}
