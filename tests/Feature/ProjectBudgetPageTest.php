...
class ProjectBudgetPageTest extends TestCase
{
    ...
    public function test_new_projects_start_with_three_core_budget_categories_and_can_add_optional_presets(): void
    {
        ...
        Livewire::test(ViewProjectBoard::class, ['record' => $project->id])
            ->call('openBasketCreate')
            ->assertSee('KA2 work package')
            ->assertSee('KA1 mobility categories')
            ->assertSee('Project Management')
            ->assertSee('Preparatory Visits')
            ->assertSee('Course Fees')
            ->assertSee('Linguistic Support')
            ->assertSee('Exceptional Costs')
            ->call('applyBasketPreset', 'ka2_project_management')
            ->assertSet('basketTitle', 'Project Management')
            ->assertSet('basketEmoji', '📋')
            ->call('applyBasketPreset', 'course_fees')
            ->assertSet('basketTitle', 'Course Fees')
            ->assertSet('basketEmoji', '🎓')
